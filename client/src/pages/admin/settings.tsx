...
const shippingAreaSchema = z.object({
  ...
  deliveryCharge: z
    .string()
    .regex(/^\d+(\.\d{1,2})?$/, { message: "Valid amount required" }),
  minDeliveryTimeHours: z
    .string()
    .regex(/^\d+$/, { message: "Valid hours required" }),
  maxDeliveryTimeHours: z
    .string()
    .regex(/^\d+$/, { message: "Valid hours required" }),
});
...
function SettingsPage() {
  ...
  const updateGeneralSettingsMutation = useMutation({
    mutationFn: async (data: GeneralSettingsFormValues) => {
      return apiRequest("PATCH", "/api/admin/settings/general", data).then(
        (res) => res.json()
      );
    },
    onSuccess: () => {
  ...
  const updateMailSettingsMutation = useMutation({
    mutationFn: async (data: MailSettingsFormValues) => {
      return apiRequest("PATCH", "/api/admin/settings/mail", data).then((res) =>
        res.json()
      );
    },
    onSuccess: () => {
  ...
  const createShippingAreaMutation = useMutation({
    mutationFn: async (data: ShippingAreaFormValues) => {
      return apiRequest(
        "POST",
        "/api/admin/settings/shipping-areas",
        data
      ).then((res) => res.json());
    },
    onSuccess: () => {
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                The name of your store, shown in the browser
                                title
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Physical address of your store, shown on contact
                              page
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                              ...
                              {shippingAreas.map((area: any) => (
                                ...
                                >
                                  ...
                                  <Button
                                    ...
                                    size="icon"
                                    onClick={() =>
                                      handleDeleteShippingArea(area._id)
                                    }
                                    disabled={
                                      deleteShippingAreaMutation.isPending
                                    }
                                  >
